...
const IntendedLearners = () => {
  ...
  const handleAddText = () => {
    if (textList.length >= 5) {
      notificationMessage("error", "Tối đa 5 đối tượng");
    } else {
  ...
  const handleDeleteText = () => {
    if (textList.length === 1) {
      notificationMessage("error", "Tối thiểu 1 đối tượng");
    } else {
  ...
  useEffect(() => {
    ...
    id &&
      teacherApi
        .getCourseDetails(id)
        .then((res: any) => {
          const { slug: _slugCourse, intendedLearners } = res.course;
          dispatch(isSuccess());
          setSlug(_slugCourse);
          setTextList(intendedLearners);
        })
